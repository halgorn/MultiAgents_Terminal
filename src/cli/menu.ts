...
import { buildMainItems, MAIN_ITEMS, type MenuAction } from './menu-items.js';
...
// ── Main menu items (defined in ./menu-items.ts) ──────────────────────────────
...
export async function runMenu(cwd: string): Promise<void> {
  ...
  while (true) {
    ...
    if (action === 'mcp-install') {
      const clientChoice = await selectOne('Install MCP for which client?', [
      ...
      if (clientChoice) {
        run(['--cwd', cwd, 'mcp', 'install', '--client', clientChoice]);
        await pressAnyKey();
      }
      continue;
    }
